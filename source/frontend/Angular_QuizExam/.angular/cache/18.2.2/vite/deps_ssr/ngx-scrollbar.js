...
import {
  ...
  ɵɵtemplate,
  ɵɵviewQuerySignal
} from "./chunk-APP3V6HK.js";
import {
  require_cjs,
  require_innerFrom
} from "./chunk-XKVJ6XJR.js";
import {
  __async,
  __spreadValues,
  __toESM
...
var _SmoothScrollManager = class _SmoothScrollManager {
  ...
  getScrollDestroyerRef(el) {
    ...
    return this.onGoingScrolls.set(el, new import_rxjs3.Subject()).get(el);
  }
  /**
  ...
  scrolling(context, destroyed) {
    return this.step(context).pipe(
      ...
      (0, import_rxjs3.takeWhile)(() => !this.isReached(context, destroyed)),
      (0, import_rxjs3.switchMap)(() => this.scrolling(context, destroyed))
    );
  }
  /**
  ...
  interrupted(el, destroyed) {
    return (0, import_rxjs3.merge)((0, import_rxjs3.fromEvent)(el, "wheel", {
      passive: true,
      capture: true
    }), (0, import_rxjs3.fromEvent)(el, "touchmove", {
      passive: true,
      capture: true
    }), destroyed).pipe((0, import_rxjs3.take)(1));
  }
    ...
    return new Promise((resolve) => {
      this.zone.runOutsideAngular(() => {
        ...
        this.scrolling(context, destroyed).pipe(
          ...
          (0, import_rxjs3.takeUntil)(this.interrupted(el, destroyed)),
          // Once finished, clean up the destroyer stream and resolve the promise
          (0, import_rxjs3.finalize)(() => this.onScrollReached(el, resolve, destroyed))
        ).subscribe();
      });
    });
  }
...
function preventSelection(doc) {
  return (0, import_rxjs5.tap)(() => doc.onselectstart = () => false);
}
function enableSelection(doc) {
  return (0, import_rxjs5.tap)(() => doc.onselectstart = null);
}
function stopPropagation() {
  return (0, import_rxjs5.tap)((e) => {
    e.preventDefault();
    e.stopPropagation();
  });
}
function getThrottledStream(stream, duration) {
  return stream.pipe((0, import_rxjs5.throttleTime)(duration || 0, null, {
    leading: false,
    trailing: true
  }));
}
      ...
      untracked(() => {
        if (viewportInit) {
          if (disableSensor) {
            ...
            this.zone.runOutsideAngular(() => {
              resizeSub$ = getThrottledStream((0, import_rxjs5.combineLatest)([this.sharedResizeObserver.observe(this.viewport.nativeElement), this.sharedResizeObserver.observe(this.viewport.contentWrapperElement)]), throttleDuration).subscribe(() => {
                this.zone.run(() => {
...
var _TrackAdapter = class _TrackAdapter extends PointerEventsAdapter {
  ...
  get pointerEvents() {
    const pointerDown$ = (0, import_rxjs5.fromEvent)(this.nativeElement, "pointerdown").pipe(stopPropagation(), preventSelection(this.document));
    const pointerUp$ = (0, import_rxjs5.fromEvent)(this.document, "pointerup", {
      passive: true
    }).pipe(enableSelection(this.document));
    const pointerEnter$ = (0, import_rxjs5.fromEvent)(this.nativeElement, "pointerover", {
      passive: true
    }).pipe(
      // When mouse is out and enters again, must set the current position first
      (0, import_rxjs5.tap)((e) => this.currMousePosition = e[this.control.offsetProperty]),
      (0, import_rxjs5.map)(() => true)
    );
    const pointerLeave$ = (0, import_rxjs5.fromEvent)(this.nativeElement, "pointerout", {
      passive: true
    }).pipe((0, import_rxjs5.map)(() => false));
    const pointerOver$ = (0, import_rxjs5.merge)(pointerEnter$, pointerLeave$).pipe((0, import_rxjs5.startWith)(true));
    const pointerMove$ = (0, import_rxjs5.fromEvent)(this.nativeElement, "pointermove", {
      passive: true
    }).pipe((0, import_rxjs5.tap)((e) => this.currMousePosition = e[this.control.offsetProperty]));
    return pointerDown$.pipe((0, import_rxjs5.switchMap)((startEvent) => {
      pointerMove$.pipe((0, import_rxjs5.takeUntil)(pointerUp$)).subscribe();
      return this.onTrackFirstClick(startEvent).pipe((0, import_rxjs5.delay)(200), (0, import_rxjs5.switchMap)(() => {
        return pointerOver$.pipe((0, import_rxjs5.switchMap)((over) => {
          const currDirection = this.getScrollDirection(this.currMousePosition);
          const sameDirection = this.scrollDirection === currDirection;
          return over && sameDirection ? this.onTrackOngoingMousedown() : import_rxjs5.EMPTY;
        }));
      }), (0, import_rxjs5.takeUntil)(pointerUp$));
    }));
  }
  constructor() {
    effect(() => {
  ...
  onTrackOngoingMousedown() {
    ...
    return this.scrollTo(position).pipe((0, import_rxjs5.takeWhile)(() => !this.isReached(position)), (0, import_rxjs5.switchMap)(() => this.onTrackOngoingMousedown()));
  }
  /**
...
var _ThumbAdapter = class _ThumbAdapter extends PointerEventsAdapter {
   ...
   */
  get pointerEvents() {
    return (0, import_rxjs5.fromEvent)(this.nativeElement, "pointerdown").pipe(stopPropagation(), preventSelection(this.document), (0, import_rxjs5.switchMap)((e) => {
      let startTrackMax;
      let startScrollMax;
      const dragStart = (0, import_rxjs5.of)(e).pipe((0, import_rxjs5.tap)(() => {
        startTrackMax = this.trackMax;
        startScrollMax = this.control.viewportScrollMax;
        this.setDragging(this.control.axis);
      }));
      const dragging = (0, import_rxjs5.fromEvent)(this.document, "pointermove").pipe(stopPropagation());
      const dragEnd = (0, import_rxjs5.fromEvent)(this.document, "pointerup", {
        capture: true
      }).pipe(stopPropagation(), enableSelection(this.document), (0, import_rxjs5.tap)(() => this.setDragging("none")));
      return dragStart.pipe((0, import_rxjs5.map)((startEvent) => startEvent[this.control.offsetProperty]), (0, import_rxjs5.switchMap)((startOffset) => dragging.pipe(
        (0, import_rxjs5.map)((moveEvent) => moveEvent[this.control.clientProperty]),
        // Calculate how far the pointer is from the top/left of the scrollbar (minus the dragOffset).
        (0, import_rxjs5.map)((moveClient) => moveClient - this.track.offset),
        (0, import_rxjs5.map)((trackRelativeOffset) => startScrollMax * (trackRelativeOffset - startOffset) / startTrackMax),
        (0, import_rxjs5.tap)((scrollPosition) => this.control.instantScrollTo(scrollPosition, startScrollMax)),
        (0, import_rxjs5.takeUntil)(dragEnd)
      )));
    }));
  }
  constructor() {
...
var _ScrollbarButton = class _ScrollbarButton extends PointerEventsAdapter {
  get pointerEvents() {
    ...
    const pointerLeave$ = (0, import_rxjs5.fromEvent)(this.nativeElement, "pointerleave", {
      passive: true
    });
    const pointerUpOrLeave$ = (0, import_rxjs5.merge)(pointerUp$, pointerLeave$);
    return pointerDown$.pipe((0, import_rxjs5.switchMap)(() => this.firstScrollStep().pipe((0, import_rxjs5.delay)(this.afterFirstClickDelay), (0, import_rxjs5.switchMap)(() => this.onOngoingPointerdown()), (0, import_rxjs5.takeUntil)(pointerUpOrLeave$))));
  }
  constructor() {
    effect(() => {
  ...
  onOngoingPointerdown() {
    return (0, import_rxjs5.interval)(0, import_rxjs5.animationFrameScheduler).pipe((0, import_rxjs5.takeWhile)(() => this.canScroll(this.control.viewportScrollOffset, this.control.viewportScrollMax)), (0, import_rxjs5.tap)(() => this.onGoingScrollStep()));
  }
};
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      untracked(() => {
        ...
        this.zone.runOutsideAngular(() => {
          sub$ = this.contentObserver.observe(this.scrollbar.nativeElement).pipe((0, import_rxjs5.throttleTime)(100, null, {
            leading: true,
            trailing: true
          })).subscribe(() => {
    ...
    effect((onCleanup) => {
      ...
      untracked(() => {
        if (!disableSensor && contentWrapperElement && spacerElement) {
          this.zone.runOutsideAngular(() => {
            sub$ = getThrottledStream(this.sharedResizeObserver.observe(contentWrapperElement), throttleDuration).pipe((0, import_rxjs5.map)((entries) => filterResizeEntries(entries, contentWrapperElement))).subscribe(() => {
              this.zone.run(() => {
                requestAnimationFrame(() => {
